...
export function StacksSection() {
  return (
    <section id="stacks" className="py-[calc(80px+4rem)] px-[clamp(1.25rem,5vw,4rem)] pb-[8rem] max-w-[1320px] mx-auto overflow-hidden">
      <ScrollReveal>
        <SectionHeader 
          number="01" 
          label="Tech Stack" 
          title="ARSENAL" 
          subtitle="Tools and technologies I use to build, deploy, and scale production systems." 
        />
      </ScrollReveal>
          ...
          >
            ...
            <div className="mt-6 flex justify-end">
              <span className="font-mono text-[0.6rem] text-muted/30 group-hover:text-accent/40 transition-colors">
                {`STATUS: READY // ${category.items.length} MODULES LOADED`}
              </span>
            </div>
          </ScrollReveal>
        ))}
